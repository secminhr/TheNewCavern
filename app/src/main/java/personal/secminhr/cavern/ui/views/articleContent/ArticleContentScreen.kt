...
class ArticleContentScreen(preview: ArticlePreview): Screen {
    ...
    override val content = @Composable {
        ...
        val article = viewModel.getArticleContent(preview) { mainActivity.showToast(it.message!!, Toast.LENGTH_SHORT) }
        ArticleContentView(
            article = article,
    ...
    override val topBarIconActions: List<() -> Unit> =
        if (MainActivity.hasCurrentUser && MainActivity.currentAccount!!.username == preview.authorUsername)
            mutableListOf({ deleteAction(preview.id) }, {}) + super.sameAppBarIconActionAs(articleScreen)
        else
